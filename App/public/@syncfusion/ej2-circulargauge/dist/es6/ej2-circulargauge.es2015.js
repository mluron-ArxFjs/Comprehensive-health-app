import { Animation, Browser, ChildProperty, Collection, Complex, Component, Event, EventHandler, Internationalization, NotifyPropertyChanges, Property, SanitizeHtmlHelper, compile, createElement, isNullOrUndefined, merge, print, remove, setStyleAttribute } from '@syncfusion/ej2-base';
...
class PointerRenderer {
     ...
     * @returns {void}
     */
    doPointerAnimation(pointerElement, pointer, axis) {
        ...
        if (pointer.animation.enable && startValue !== endValue && this.gauge.animatePointer) {
            pointer.pathElement.map((element) => {
                if (pointer.type === 'RangeBar') {
                    this.performRangeBarAnimation(element, startValue, endValue, axis, pointer);
                }
                else {
                    if (pointer.type === 'Marker' && pointer.markerShape === 'Text') {
                        this.performTextAnimation(pointerElement, startValue, endValue, axis, pointer);
                    }
                    else {
                        this.performNeedleAnimation(element, startValue, endValue, axis, pointer);
                    }
                }
            });
        }
     ...
     * @private
     */
    performTextAnimation(element, start, end, axis, pointer) {
        const isClockWise = axis.direction === 'ClockWise';
        let textangle;
        let textlocation;
        let pointerValue = 0;
     ...
     * @private
     */
    performNeedleAnimation(element, start, end, axis, pointer) {
        const isClockWise = axis.direction === 'ClockWise';
        start = typeof (start) === 'string' ? parseInt(start, 10) : start;
        end = typeof (end) === 'string' ? parseInt(end, 10) : end;
     ...
     * @private
     */
    performRangeBarAnimation(element, start, end, axis, pointer) {
        start = typeof (start) === 'string' ? parseInt(start, 10) : start;
        end = typeof (end) === 'string' ? parseInt(end, 10) : end;
        let pointerValue;
        let timeStamp;
...
class Legend {
    ...
    renderLegend(legend, legendBounds) {
        ...
        if (firstLegend !== this.legendCollection.length) {
            ...
            const start = new GaugeLocation((!this.gauge.enableRtl) ? legendBounds.x + padding + (legend.shapeWidth / 2) : (!this.isVertical) ? legendBounds.width + legendBounds.x - (padding) - (legend.shapeWidth) : legendBounds.x + this.maxWidth - padding - legend.shapeWidth / 2, legendBounds.y + padding + this.maxItemHeight / 2);
...
let CircularGauge = class CircularGauge extends Component {
    ...
    renderTitle() {
        if (this.title) {
            ...
            const size = measureText(this.title, style);
        ...
        if (!this.isDestroyed && pointer.currentValue !== value) {
            ...
            pointer.pathElement.map((element) => {
                if (pointer.type === 'RangeBar') {
                    ...
                    if (enableAnimation) {
                        this.gaugeAxisLayoutPanel.pointerRenderer.performRangeBarAnimation(element, pointer.currentValue, value, axis, pointer);
                    }
                    else {
                    ...
                    if (enableAnimation) {
                        if (pointer.type === 'Marker' && pointer.markerShape === 'Text') {
                            this.gaugeAxisLayoutPanel.pointerRenderer.performTextAnimation(element, pointer.currentValue, value, axis, pointer);
                        }
                        else {
                            this.gaugeAxisLayoutPanel.pointerRenderer.performNeedleAnimation(element, pointer.currentValue, value, axis, pointer);
                        }
                    }
                    else {
                        this.gaugeAxisLayoutPanel.pointerRenderer.setPointerValue(axis, pointer, value);
            ...
            for (const prop of Object.keys(newProp)) {
                switch (prop) {
                    ...
                    case 'axes':
                        ...
                        const axesPropertyLength = Object.keys(newProp.axes).length;
